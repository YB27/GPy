...
class SVGPCensored(SparseGP):
    def __init__(self, X, Y, Z, lowerThreshold, upperThreshold, kernel, mean_function=None, name='SVGPCensored', lik_var=1. , Y_metadata=None, batchsize=None, num_latent_functions=None):
        """
        ...
        likelihood = likelihoods.Tobit(lowerThreshold=lowerThreshold, upperThreshold=upperThreshold, variance=lik_var)
        self.batchsize = batchsize
        self.X_all, self.Y_all, self.Y_metadata_all = X, Y, Y_metadata
        if batchsize is None:
            X_batch, Y_batch, Y_metadata_batch = X, Y, Y_metadata
        else:
            import climin.util
            ...
            # Here we also need to take the corresponding batch for the metadata (indexes for censored data)
            X_batch, Y_batch, Y_metadata_batch = self.new_batch()

        #create the SVI inference method
        inf_method = svgp_inf()

        super(SVGPCensored, self).__init__(X_batch, Y_batch, Z, kernel, likelihood, mean_function=mean_function, inference_method=inf_method,
                 name=name, Y_metadata=Y_metadata_batch, normalizer=False)
    ...
    def set_data(self, X, Y, Y_metadata):
        """
        ...
        self.X, self.Y, self.Y_metadata = X, Y, Y_metadata

    def new_batch(self):
        """
        Return a new batch of X and Y by taking a chunk of data from the complete X and Y
        In this censored version, also return the batch for Y_metadata
        """
        i = next(self.slicer)
        Y_metadata_batch = {}
        for key in self.Y_metadata_all.keys():
            # !! Here Y_metadata considers the local batch indexes !
            Y_metadata_batch[key] = np.array([idx - i.start for idx in self.Y_metadata_all[key] if idx >= i.start and idx < i.stop])

        return self.X_all[i], self.Y_all[i], Y_metadata_batch
